...
using Yurowm.Properties;
using Yurowm.Utilities;

namespace YMatchThree.Editor {
    [CustomEditor(typeof(SlotRendererLegacy))]
    public class SlotRendererEditor : UnityEditor.Editor {

        SlotRendererLegacy provider;
        ...
        void OnEnable() {
            provider = (SlotRendererLegacy) target;
        ...
        public override void OnInspectorGUI() {
            ...
            using (GUIHelper.Change.Start(provider.RebuildImmediate)) {
                ...
                provider.color = EditorGUILayout.ColorField("Color", provider.color);
                provider.innerMesh = EditorGUILayout.Toggle("InnerMesh", provider.innerMesh);
            
                using (GUIHelper.Vertical.Start(GUIHelper.DrawSeparatorRect, GUILayout.ExpandWidth(true))) {
                ...
                using (GUIHelper.Vertical.Start(GUIHelper.DrawSeparatorRect, GUILayout.ExpandWidth(true))) {
                    ...
                    provider.sizeRight = EditorGUILayout.Slider("Right", provider.sizeRight, 0, 1f);
                    provider.sizeLeft = EditorGUILayout.Slider("Left", provider.sizeLeft, 0, 1f);
                    provider.sizeTop = EditorGUILayout.Slider("Top", provider.sizeTop, 0, 1f);
                    provider.sizeBottom = EditorGUILayout.Slider("Bottom", provider.sizeBottom, 0, 1f);
                }

                using (GUIHelper.Vertical.Start(GUIHelper.DrawSeparatorRect, GUILayout.ExpandWidth(true))) {
                    GUILayout.Label("Border UV", Styles.centeredMiniLabel);

                    using (GUIHelper.Change.Start(() => editedRect = GetRect(editedRectName)))
                        editedRectName = rectNames[EditorGUILayout.Popup("Area", Mathf.Max(0, rectNames.IndexOf(editedRectName)), rectNames.ToArray())];

                    using (GUIHelper.Change.Start(() => SaveRect(editedRectName))) {
        ...
        static float magnetCellSize {
            get => EditorStorage.Instance.GetFloat("BREditorMagnet");  
            set => EditorStorage.Instance.SetNumber("BREditorMagnet", value);
        }
        ...
        void DrawPreview() {
            ...
            magnetCellSize = EditorGUILayout.Slider("Magnet Cell Size", magnetCellSize, 0, .5f);
            ...
            foreach (string name in rects.Keys) {
                ...
                using (GUIHelper.Color.Start(rectBorderColor))
            ...
            if (editedRectName != "") {
                ...
                using (GUIHelper.Color.Start(editedRectBorderColor))
                    GUI.Box(new Rect(r.x, r.y, r.width, 16), editedRectName, rectLabel);

                Vector2 delta;
                using (GUIHelper.Change.Start(() => {
        ...
        Rect GetRect(string name) {
            return (Rect) typeof(SlotRendererLegacy).GetField(rects[name]).GetValue(provider);
        }

        void SaveRect(string name) {
            typeof(SlotRendererLegacy).GetField(rects[name]).SetValue(provider, editedRect);
        }
