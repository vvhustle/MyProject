...
namespace Yurowm.Controls {
    public class TouchControl {
        ...
        List<TouchStory> touches = new List<TouchStory>();
        ...
        void TouchControls() {
            ...
            foreach (Touch unityTouch in Input.touches) {
                ...
                if (unityTouch.phase == TouchPhase.Began) {
                    if (IsOverUI(unityTouch.fingerId))
                        continue;
                    ...
                    touch.IsOverUI = IsOverUI(touch.fingerId);
            ...
            DebugPanel.Log("Touches", "Input", touches.Select(t => $"{t.fingerId}. ({t.currentScreenPosition})").Join("; "));
            #endif
        }
    ...
    public class Clickables {
        // TODO: Сделать это объектом Space
        List<ICastable> all = new List<ICastable>();
        ...
        public C Cast<C>(Vector2 position, float threashold) where C : ICastable {
            ...
            var target = all.Where(c => c is C && c.IsAvaliableForClick(controlMode) 
                                               && (c.position - position).MagnitudeIsLessThan(threashold))
                .GetMin(c => Vector2.Distance(c.position, position));
            return (C) target;
        }
