...
namespace Yurowm.Nodes.Editor {
    public class NodeSystemEditor {
        ...
        void CanvasContextMenu(Vector2 position) {
            ...
            if (nodeReference != null && supportedTypes.Any(t => t.IsInstanceOfType(nodeReference)))
                menu.AddItem(new GUIContent("Paste"), false, () => {
                    var node = nodeReference.Clone();
                    AddNode(node);
                    node.position = position;
                });
    ...
    public class NodeSystemEditorWindow : EditorWindow {
        ...
        public static NodeSystemEditorWindow Show(NodeSystem system) {
